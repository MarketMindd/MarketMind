...
import { PortfolioStock, mockStocks, availableStocksForPortfolio } from '@/data/mockData';
import { Button } from '@/components/ui/button';
import { Briefcase, TrendingUp, TrendingDown, DollarSign, Save, Check } from 'lucide-react';
...
interface PortfolioProps {
  ...
  onPortfolioChange: (portfolio: PortfolioStock[]) => void;
}

const Portfolio = ({ onLogout, portfolio, onPortfolioChange }: PortfolioProps) => {
  const [localPortfolio, setLocalPortfolio] = useState<PortfolioStock[]>(portfolio);
  ...
  const handleSave = () => {
    onPortfolioChange(localPortfolio);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-24 sm:pt-20 pb-12 px-4 sm:px-6 lg:px-8 max-w-4xl mx-auto">
        {/* Header */}
        <div className="flex items-center justify-between mb-8 animate-fade-in">
          ...
          {hasChanges && (
            <Button variant="glow" onClick={handleSave} className="animate-fade-in">
              <Save size={18} />
              Save Changes
            </Button>
          )}
