...
import { Mail, Lock, User, ArrowRight, Briefcase, ChevronDown, ChevronUp, FileSpreadsheet, Upload } from 'lucide-react';
...
export const SignUp: React.FC = () => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const mockImportedStocks: PortfolioItem[] = [
      { ticker: 'AAPL', shares: 50, avgPrice: 165.00 },
      { ticker: 'MSFT', shares: 30, avgPrice: 350.00 },
      { ticker: 'GOOGL', shares: 20, avgPrice: 140.00 },
    ].filter(stock => availableStocks.some(s => s.symbol === stock.ticker));

    setPortfolio(prev => {
      const existingTickers = new Set(prev.map(p => p.ticker));
      const newStocks = mockImportedStocks.filter(s => !existingTickers.has(s.ticker));
      return [...prev, ...newStocks];
    });

    toast({
      title: 'Portfolio imported',
      description: `Successfully imported ${mockImportedStocks.length} stocks from your file.`,
    });
      ...
      <div className="flex-1 flex items-center justify-center p-8 overflow-y-auto">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" variant="glow" size="lg" className="w-full" disabled={signUp.isPending}>
